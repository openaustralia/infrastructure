...
# Generates certificates for local development based off myCA.{key,pem}, A local root certificate,
# that is unique to you, and can be added to your browser
...
if [ -d ../../morph/provisioning/roles/morph-app ]; then
  ...
  mv morph.${BASE_DOMAIN}.key morph.${BASE_DOMAIN}.pem ../../morph/provisioning/roles/morph-app/files/ssl
  set +x
else
