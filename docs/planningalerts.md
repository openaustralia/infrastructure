...
We now have two production hosts, and a blue/green cutover process driven
...
`terraform/main.tf` to update the environment that isn't currently taking
traffic; then tweak values in the blue/green modules in
`terraform/planningalerts/main.tf` to adjust where traffic is going. Don't
forget that you'll need to `terraform apply` at each step of the change.
